        ...
        <div className={`transition-all ${navBg ? 'bg-[#0f142ed9] shadow-md' : 'fixed'} duration-200 h-[12vh] z-[10000] fixed w-full`}>
            <div className='flex items-center h-full justify-between w-[90%] mx-auto'>
                <div className='flex items-center space-x-2'>
                    ...
                    <h1 className='text-xl hidden sm:block md:text-2xl text-white font-bold'>Vigneshwaran Balasubramani</h1>
                </div>
                <div className='hidden lg:flex items-center space-x-10'>
                    {NAVLINKS.map((link) => {
                        return <span key={link.id} onClick={() => onNavClick(link.url)} className='text-base hover:text-cyan-300 text-white font-medium transition-all duration-200 cursor-pointer'>
                            {link.label}
                        </span>
                    })}
                </div>
